    ...
    <view class="comments-view">
        ...
        <view class="evaluate-class" v-if="labeldata.length > 0">
            <block v-for="(item,index) in labeldata" :key="index">
                <text v-if="item.num == '' " :class="{actives:num == index}"
                    @click="labelFun(item.commid,item.label,index)">{{item.label}}</text>
                <text v-else :class="{actives:num == index}"
                    @click="labelFun(item.commid,item.label,index)">{{item.label}}({{item.num}})</text>
            </block>
        </view>
        <!-- 评价 -->
    ...
    export default {
        ...
        methods: {
            async reqComm(id) {
                ...
                let comtconent = new this.Request(this.Urls.m().comtconent + '?id=' + id + '&label=' + '全部')
                    .modeget()
                try {
