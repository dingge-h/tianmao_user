        ...
        <view class="shopp-view" v-if="emcart">
                    ...
                    <view class="shopp-goods">
                        ...
                        <view class="shopp-goods-view">
                            <view class="shopp-chick">
                                <image v-if="item.choice" @click="seLect(item._id,'unchecked')"
                                    src="/static/details/xuanzhong.svg" mode="widthFix"></image>
                                <image v-else @click="seLect(item._id,'select')" src="/static/details/weixuan.svg"
                                    mode="widthFix"></image>
                            </view>
                            ...
                            <view class="shopp-chick-gight">
                                <view class="shopp-chick-right">
                                    ...
                                    <view class="shopp-chick-sku"
                                        @click="normsFun(item.id,item._id,item.size,item.color,item.many)">
                                        <text>{{item.size}};</text>
                                ...
                                <view class="shopp-price">
                                    ...
                                    <view class="shopp-price-nums">
                                        <view
                                            @click="item.many === 1 ? false : true && reDuce(item._id,item.many,item.price)">
                                            -</view>
                                        <view>{{item.many}}</view>
            ...
            <view class="settlement-view">
                <view class="settlement-img">
                    <image v-if="!selectall" src="/static/details/weixuan.svg" mode="widthFix"
                        @click="selectAll('seall')"></image>
                    <image v-if="selectall" src="/static/details/xuanzhong.svg" mode="widthFix"
                        @click="selectAll('deall')"></image>
                </view>
                ...
                <view class="settlement-money" v-show="settips" @click="total.disable && semEnt()">
                    结算({{mapselect.length}})</view>
    ...
    export default {
        ...
        methods: {
            ...
            async deLece() {
                ...
                try {
                    await new this.Request(this.Urls.m().cartdelete, {
                        arrid: this.cartdele
                    }).modepost()
                    this.myCart()
                    uni.hideLoading()
            ...
            async seLect(id, nums) {
                ...
                try {
                    await new this.Request(this.Urls.m().selecturl, {
                        id,
                        nums
                    }).modepost()
                    this.myCart()
